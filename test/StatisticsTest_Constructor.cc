...
/**
 * The copy constructor should also copy the sets from the old relation
 */
TEST_F(StatisticsTest, Constructor4) {
	// Add Relations
	map["rel1"].numTuples = 1;
	map["rel2"].numTuples = 2;
	map["rel3"].numTuples = 3;
	...
	// Add set
	map["rel1"].set.insert("rel1");
	map["rel1"].set.insert("rel2");
	map["rel2"].set.insert("rel1");
	map["rel2"].set.insert("rel2");
	...
	EXPECT_EQ(copy.NumDistincts("rel3", "att8"), 8);
	EXPECT_EQ(copy.NumDistincts("rel3", "att9"), 9);
	EXPECT_EQ(1, copy.GetSet("rel1").count("rel1"));
	EXPECT_EQ(1, copy.GetSet("rel1").count("rel2"));
	EXPECT_EQ(1, copy.GetSet("rel2").count("rel1"));
	EXPECT_EQ(1, copy.GetSet("rel2").count("rel2"));
}
