...
/**
 * Read should update the lookup table when adding an attribute and put each relation in its
 * own set
 */
TEST_F(StatisticsTest, Read5) {
	StrictMock<MockRawFile> file;
	InSequence seq;
	EXPECT_CALL(file, Open("file.txt"));
	EXPECT_CALL(file, ReadLine(NotNull())).
	...
	Read("file.txt", file);
	EXPECT_EQ(1, map["rel1"].set.count("rel1"));
	EXPECT_EQ(0, stat.RelLookup("hey").compare("rel1"));
}
