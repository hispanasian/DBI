...
public:
	...
	virtual int Compare(Record *left, Record *right, OrderMaker *orderUs);
	...
	// Compares over the first m attributes of the two OrderMakers, where m is the mininum of the 
	// number of attributes of the two OrderMakers
	...
	virtual int Compare(Record *left, Record *right, Record *literal, CNF *myComparison);

	// like the last one, but for unary operations
	virtual int Compare(Record *left, Record *literal, CNF *myComparison);
